...
*/
(function () {
  const doc = document; let PIE = window.PIE;
 ...
 */
    (function () {
      ...
      PIE.Util = {
         ...
         */
        createVmlElement(tag) {
          const vmlPrefix = 'css3vml';
          if (!vmlCreatorDoc) {
      ...
      perpendicularIntersect(x1, y1, angle, x2, y2) {
        ...
        if (angle === 0 || angle === 180) {
          ...
          return [x1, y2];
            ...
            // the same x/y values.
        let a1 = Math.tan(-angle * Math.PI / 180),
          c1 = a1 * x1 - y1,
          a2 = -1 / a1,
          c2 = a2 * x2 - y2,
          d = a2 - a1,
 ...
 */
    ...
    PIE.OnUnload.attachManagedEvent(window, 'onresize', () => { PIE.OnResize.fire(); });
/**
      ...
      Length.prototype = {
        ...
        getEmPixels(el) {
          ...
          if (fs.indexOf('px') > 0) {
            ...
            return PIE.getLength(fs).pixels(parent, () => {
 ...
 */
    PIE.VisibilityStyleInfo = PIE.StyleInfoBase.newStyleInfo({
      ...
      getCss: PIE.StyleInfoBase.cacheWhenLocked(function () {
        const cs = this.targetElement.currentStyle;
     ...
     */
      getShape(name, subElName, parent, group) {
     ...
     */
      getBox() {
        let box = this.parent.getLayer(this.boxZIndex),
          s;
 ...
 */
    PIE.RootRenderer = PIE.RendererBase.newRenderer({
      ...
      updateVisibility() {
        const vis = this.styleInfos.visibilityInfo.getProps();
 ...
 */
     ...
     * @param {number} index
     */
      positionBgImage(shape, index) {
        const me = this;
        PIE.Util.withImageSize(shape.fill.src, (size) => {
          let el = me.targetElement,
 ...
 */
    PIE.BorderImageRenderer = PIE.RendererBase.newRenderer({
      ...
      draw() {
        ...
        let props = this.styleInfos.borderImageInfo.getProps(),
    ...
    PIE.BoxShadowOutsetRenderer = PIE.RendererBase.newRenderer({
      ...
      draw() {
        ...
        function getShadowShape(index, corner, xOff, yOff, color, blur, path) {
          let shape = me.getShape(`shadow${index}${corner}`, 'fill', box, len - index),
            fill = shape.fill;
 ...
 */
    PIE.IE9BackgroundRenderer = PIE.RendererBase.newRenderer({
      ...
      bgPositionToString(bgPosition) {
        return bgPosition ? bgPosition.tokens.map((token) => {
          return token.tokenValue;
        }).join(' ') : '0 0';
      },

      getBgAreaSize(bgOrigin) {
        let me = this,
          el = me.targetElement,
          bounds = me.boundsInfo.getBounds(),
          elW = bounds.w,
      ...
      getGradientSvg(info, bgWidth, bgHeight) {
        let el = this.targetElement,
