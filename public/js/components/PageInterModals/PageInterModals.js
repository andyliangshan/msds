    ...
    return (
      <div className="adBg" style={{ display: this.state.currentModal === -1 ? 'none' : 'block' }}>
        ...
        <div className="bg-pic-img bg-pic3 bm1" style={{ display: this.state.currentModal === 2 ? 'block' : 'none', background: `url(${baogaoImg}) no-repeat center` }}>
          ...
          <div className="toclose" onClick={(evt) => this.setModalLoop(evt, 2, this.props.time4)} data-growing-title="我不关心"></div>
        </div>
        <div className="bg-pic-img bg-pic4 bm1" style={{ display: this.state.currentModal === 3 ? 'block' : 'none' }}>
          <Live800 classes={'live800 live8001 bg-pic'} title={''} type={'a'} tips={'弹窗1'} data-growing-title={'挑选顾问'} />
          <Live800 classes={'live800 live8001 bg'} title={''} type={'a'} tips={'挑选顾问'} data-growing-title={'挑选顾问'} />
